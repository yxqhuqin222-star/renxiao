...
本地 Flask 页面点击「上传并更新」后，会在数据入库成功时自动执行只读快照发布：

1. 导出 `docs/index.html`
2. 只暂存并提交 `docs/index.html`
3. 推送 `main`，由 GitHub Pages 更新公开页面

如果数据没有变化，会跳过提交；如果导出、提交或推送失败，本地数据仍会保留，并在页面顶部提示公开页未同步成功。
